...
class ScraperAlbert(Scraper): # Scan takes 150s
	...
	def send_persistent_query(data: dict) -> Tuple[bool, requests.Response, Any]:
		resp = requests.get("https://www.albert.cz/api/v1/?" + ScraperAlbert.build_url(data))
		resp_json = resp.json()
		if "errors" in resp:
	...
	def send_query(data: dict, query: str) -> Tuple[requests.Response, Any]:
		found, resp, resp_json = ScraperAlbert.send_persistent_query(data)
		if found:
			return resp, resp_json
		print("Hash not found. Sending full query.")
		data["query"] = query
		resp = requests.post("https://www.albert.cz/api/v1/", json=data)
				...
				try:
					for k in resp_json["data"]["categoryProductSearch"]["products"]:
						if category_url == k["url"].split('/')[2]: # This is to exclude duplicates. Let me know if you know of a better way to do this
							item = ScraperAlbert.parse_item(k, category_name)
							if item:
								append_record(item.__dict__)
				except Exception as e:
					raise Exception(f"Invalid response at [{resp.url}]:\n{resp.text}\nException:\n{traceback.format_exc()}")
	
	def parse_item(item, category, timestamp:int=None) -> Optional[Item]:
		i = Item()
		i.name = item["name"]
		i.category = category
