...
def check_existence(record):
	...
	if not os.path.exists(get_filename(record["store"],record["id"])):
		with open(get_filename(record["store"],record["id"]), "x", newline='\n') as f:
...
def get_last_state(store, id):
	with open(get_filename(store,id), "r", newline='\n') as f:
...
def append_record(record):
	...
	with open(get_filename(record["store"],record["id"]), "a", newline='\n') as f:
...
def append_multiple_records(records):
	...
	with open(get_filename(records[0]["store"],records[0]["id"]), "a", newline='\n') as f:
		writer = csv.DictWriter(f, fieldnames=PROPS, **CSV_CONF)
